...
This module provides a unified interface for MongoDB Atlas (primary) and
SQLite (fallback) database operations. It automatically handles connection
...
def _get_mongo_collection() -> Optional[Collection]:
    ...
    try:
        ...
        mongo_errors.ConfigurationError,
    ) as e:
        ...
        try:
            ...
            if collection is not None:
                ...
                collection.update_one(
                    {"line_user_id": user_id}, {"$set": user_doc}, upsert=True
                )

                logger.info(
                    f"Successfully added user {user_id} ({username}) to MongoDB"
                )
...
def remove_user(user_id: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if collection is not None:
                ...
                result = collection.update_one(
                    {"line_user_id": user_id}, {"$set": {"authorized": False}}
                )
    ...
    try:
        ...
        cursor.execute(
            "UPDATE allowed_users SET authorized = 0 WHERE user_id = ?", (user_id,)
        )
