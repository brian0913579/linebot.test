...
from config.config_module import RATE_LIMIT_ENABLED, MAX_REQUESTS_PER_MINUTE
...
def limit_webhook_endpoint(app):
    ...
    limiter.limit(f"{MAX_REQUESTS_PER_MINUTE} per minute")(app.view_functions["webhook"])
    logger.info(f"Applied rate limit to webhook endpoint: {MAX_REQUESTS_PER_MINUTE} requests per minute")
...
def limit_verify_location_endpoint(app):
    ...
    logger.info(f"Applied rate limit to verify-location endpoint: {twice_limit} requests per minute")