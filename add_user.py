...
from utils.mongodb_client import add_user, get_user, update_user_authorization, remove_user
...
Examples:
  ...
  Remove a user:
    ...
    add_parser.add_argument('line_user_id', help='LINE user ID (e.g., U1234567890abcdef)')
    add_parser.add_argument('username', help='Display name for the user')
    add_parser.add_argument('--unauthorized', action='store_true', 
                           help='Add user but mark as unauthorized')
    ...
    try:
        if args.command == 'add':
            ...
            if success:
                ...
                print(f"❌ Failed to add user. User may already exist.")
                return 1
            ...
            if success:
                ...
                print(f"❌ Failed to authorize user. User may not exist.")
                return 1
            ...
            if success:
                print(f"✅ User deauthorized successfully!")
                return 0
            else:
                print(f"❌ Failed to deauthorize user. User may not exist.")
                return 1
            ...
            if success:
                ...
                print(f"❌ Failed to remove user. User may not exist.")
                return 1
