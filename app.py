...
def is_allowed_user(user_id):
    if user_id not in ALLOWED_USERS:
        app.logger.error(f"Unauthorized user attempt: {user_id}")
        return False
    return True
...
def handle_text(event):
    try:
        ...
        if not is_allowed_user(user_id):
            send_message(event.reply_token, "❌ 您尚未註冊為停車場用戶，請聯絡管理員。")
            return
        ...
        send_message(event.reply_token, "❌ 系統錯誤，請稍後再試。")
...
def handle_location(event):
    try:
        user_id = event.source.user_id
        if not is_allowed_user(user_id):
            send_message(event.reply_token, "❌ 未授權使用者，請勿嘗試操作。")
            return
...
def handle_postback(event):
    try:
        ...
        if action == 'open':
            ...
            send_message(event.reply_token, "✅ 門已開啟，請小心進出。")
        else:
            # TODO: insert GPIO close logic
            send_message(event.reply_token, "✅ 門已關閉，感謝您的使用。")
