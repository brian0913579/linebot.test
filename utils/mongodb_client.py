...
def get_mongo_client() -> MongoClient:
    ...
    if _mongo_client is None:
        ...
        if not mongo_uri:
            raise RuntimeError(
                "MONGO_URI not configured. Please set the MONGO_URI environment variable "
                "or configure it in Google Cloud Secret Manager."
            )
        ...
        try:
            ...
            logger.info("MongoDB connection established successfully with connection pooling")
...
def get_allowed_users() -> Dict[str, str]:
    ...
    try:
        ...
        users = collection.find(
            {"authorized": True},
            {"line_user_id": 1, "username": 1, "_id": 0}
        )
        
        # Convert to dictionary
        allowed_users = {user["line_user_id"]: user["username"] for user in users}
        
        logger.info(f"Successfully retrieved {len(allowed_users)} authorized users from MongoDB")
...
def add_user(line_user_id: str, username: str, authorized: bool = True) -> bool:
    ...
    try:
        ...
        if result.inserted_id:
            logger.info(f"Successfully added user {username} ({line_user_id}) to MongoDB")
            return True
        else:
...
def update_user_authorization(line_user_id: str, authorized: bool) -> bool:
    ...
    try:
        ...
        if result.modified_count > 0:
            logger.info(f"Updated authorization for user {line_user_id} to {authorized}")
            return True
        else:
