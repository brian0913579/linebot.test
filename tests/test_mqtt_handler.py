...
def test_on_connect_failure(caplog):
    ...
    assert any("Failed to connect to MQTT broker" in record.message for record in caplog.records)
    assert any("bad username or password" in record.message for record in caplog.records)
...
def test_on_disconnect_unexpected(caplog):
    ...
    assert any("Unexpected disconnection" in record.message for record in caplog.records)