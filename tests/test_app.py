...
from config.config_module import MAX_DIST_KM, PARK_LAT, PARK_LNG
from core.line_webhook import verify_signature
...
def create_test_signature(body, secret):
    ...
    return base64.b64encode(
        hmac.new(key=secret, msg=body.encode(), digestmod=hashlib.sha256).digest()
    ).decode()
...
def test_verify_location_endpoint_invalid_token(client):
    ...
    response = client.post(
        "/api/verify-location?token=invalid_token",
